...
Revision ID: b75b7e61adb4
Revises: 481f4f0c6047
Create Date: 2019-09-19 20:19:18.406502
...
revision = 'b75b7e61adb4'
down_revision = '481f4f0c6047'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    sa.Column('owner_id', sa.dialects.postgresql.UUID, nullable=True),
...
def downgrade():
    ...
    op.create_table('recognition_opencv_origem',
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.Column('id', postgresql.UUID(), autoincrement=False, nullable=False),
    sa.Column('name', sa.VARCHAR(length=24), autoincrement=False, nullable=False),
    sa.Column('cnpjcpf', sa.VARCHAR(length=14), autoincrement=False, nullable=False),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.PrimaryKeyConstraint('id', name='recognition_opencv_uservalidacao_pkey'),
    sa.UniqueConstraint('cnpjcpf', name='recognition_opencv_uservalidacao_cpf_key'),
    sa.UniqueConstraint('name', name='recognition_opencv_uservalidacao_id_urpay_key'),
    postgresql_ignore_search_path=False
    )
    op.create_index('recognition_opencv_uservalidacao_id_urpay_2994aff5_like', 'recognition_opencv_origem', ['name'], unique=False)
    op.create_index('recognition_opencv_uservalidacao_cpf_ed042bf9_like', 'recognition_opencv_origem', ['cnpjcpf'], unique=False)
    ...
    op.create_table('recognition_opencv_validacao',
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.Column('id', postgresql.UUID(), autoincrement=False, nullable=False),
    sa.Column('user_id', sa.VARCHAR(length=50), autoincrement=False, nullable=False),
    sa.Column('document_type', sa.VARCHAR(length=4), autoincrement=False, nullable=False),
    sa.Column('percent_similarity_urpay', sa.VARCHAR(length=20), autoincrement=False, nullable=False),
    sa.Column('percent_similarity_datavalid', sa.VARCHAR(length=20), autoincrement=False, nullable=True),
    sa.Column('datavalid_probability', sa.VARCHAR(length=30), autoincrement=False, nullable=True),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    sa.Column('origem_validation_fk_id', postgresql.UUID(), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['origem_validation_fk_id'], ['recognition_opencv_origem.id'], name='recognition_opencv_v_origem_validation_fk_09f1b531_fk_recogniti', initially='DEFERRED', deferrable=True),
    sa.PrimaryKeyConstraint('id', name='recognition_opencv_validacao_pkey')
    )
    op.create_index('recognition_opencv_validacao_origem_validation_fk_id_09f1b531', 'recognition_opencv_validacao', ['origem_validation_fk_id'], unique=False)
